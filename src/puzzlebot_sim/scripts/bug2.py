...
class GoToGoal(): 
    def __init__(self): 
        ...
        fw_distance = 0.395 # distance to activate the following walls behavior [m]
        ...
        if(self.x_target == 0.0):
            ...
            self.m = (self.y_target-self.robot.y)/(self.x_target-self.robot.x)
        ...
        while not rospy.is_shutdown(): 
            ...
            if self.lidar_received:
                ...
                d_t=np.sqrt((self.x_target-self.robot.x)**2+(self.y_target-self.robot.y)**2)    # distance to goal

                expected_y = self.m*self.robot.x
                expected_x = self.robot.y / self.m

                print("Robot y:"+str(self.robot.y),"Calculated y:"+str(expected_y))
                print("Robot x:"+str(self.robot.x),"Calculated x:"+str(expected_x))
                if self.at_goal(): 
                    print("Goal reached")
                    ...
                    # print("Robot y:"+str(self.robot.y),"Calculated y:"+str(expected_y))
                    # print("Robot x:"+str(self.robot.x),"Calculated x:"+str(expected_x))
                    #if (self.robot.y >= expected_y-0.1) and ( self.robot.y <= expected_y+0.1): #ADD output condition#:
                    #if self.m == 0.0:
                    #    if ((self.robot.x >= expected_x-0.1) and ( self.robot.x <= expected_x+0.1)) and (self.robot.y != self.y_target):
                    #        self.current_state = 'GoToGoal'
                    #        print("Change to Go to goal")
                    if ((self.robot.y >= abs(expected_y-0.1)) and ( self.robot.y <= abs(expected_y+0.1))): #ADD output condition#:
                        self.current_state = 'GoToGoal'
                        print("Change to Go to goal")
                    else:
    ...
    def goal_cb(self, goal): 
        ...
        self.m = (self.y_target-self.robot.y)/(self.x_target-self.robot.y)
